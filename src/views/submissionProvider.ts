...
export class SubmissionProvider implements vscode.TreeDataProvider<SubmissionTreeItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<SubmissionTreeItem | undefined | null | void> = new vscode.EventEmitter<SubmissionTreeItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<SubmissionTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    async getChildren(element?: SubmissionTreeItem): Promise<SubmissionTreeItem[]> {
         if (!this.authService.isLoggedIn()) {
             return [new vscode.TreeItem("Please login to view submissions", vscode.TreeItemCollapsibleState.None) as SubmissionTreeItem];
         }
        ...
        if (element) {
            ...
            try {
                ...
                const { submissions } = await this.apiClient.getSubmissions(undefined , username);
                return submissions.map(s => new SubmissionTreeItem(s));
                // TODO: implement pagination
            } catch (error: any) {
                vscode.window.showErrorMessage(`Failed to load submissions: ${error.message}`);
                 return [new vscode.TreeItem(`Error: ${error.message}`, vscode.TreeItemCollapsibleState.None) as SubmissionTreeItem];
            }
        }
    }
}
...
export class SubmissionTreeItem extends vscode.TreeItem {
    constructor(public readonly submission: SubmissionBrief) {
        const problemTitle = submission.problem?.title || `Problem ${submission.problem?.id || '?'}`;
    ...
    static getIconForStatus(status: SubmissionStatus): vscode.ThemeIcon {
        switch (status) {
            ...
            case 'disk_limit_exceeded':
                return new vscode.ThemeIcon('clock', new vscode.ThemeColor('testing.iconSkipped'));
            case 'pending':
            case 'compiling':
            case 'judging':
