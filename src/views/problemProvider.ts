...
export class ProblemProvider implements vscode.TreeDataProvider<ProblemTreeItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<ProblemTreeItem | undefined | null | void> = new vscode.EventEmitter<ProblemTreeItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<ProblemTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    async getChildren(element?: ProblemTreeItem): Promise<ProblemTreeItem[]> {
        if (!this.authService.isLoggedIn()) {
             return [new vscode.TreeItem("Please login to view problems", vscode.TreeItemCollapsibleState.None) as ProblemTreeItem];
        }
        ...
        if (element) {
            ...
            try {
                ...
                return [new vscode.TreeItem(`Error: ${error.message}`, vscode.TreeItemCollapsibleState.None) as ProblemTreeItem];
            }
        }
    }
}
