...
export class AuthService {
    ...
    constructor(private context: vscode.ExtensionContext) {
        ...
        this.loadTokenFromStorage().then(loaded => {
            ...
            if (loaded && !this._profile) { // If token exists but profile missing, try fetching
    ...
    private async storeAndValidateToken(token: string): Promise<boolean> {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`ACMOJ Token set for ${profile.friendly_name} (${profile.username})!`);
                success = true; // Mark as successful

            } catch (error: any) {
                vscode.window.showErrorMessage(`Failed to set or validate token: ${error.message}. Please check the token.`);
    ...
    public async clearToken(): Promise<void> {
        ...
        if (wasLoggedIn) {
            vscode.window.showInformationMessage('ACMOJ Personal Access Token cleared.');
        } else {
    ...
    private async loadTokenFromStorage(): Promise<boolean> {
        try {
            ...
            const storedProfile = this.context.globalState.get<Profile | null>(PROFILE_KEY, null); // Load profile too
    ...
    public async handleUnauthorizedError(): Promise<void> {
        ...
        if (currentToken) { // Only show message if a token was actually set
             vscode.window.showErrorMessage('ACMOJ: Invalid or expired Personal Access Token. Please set a new token.', 'Set Token')
                .then(selection => {
