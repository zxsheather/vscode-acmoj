...
import { ProblemBrief, Problem, SubmissionBrief, Submission, ApiError, Profile, Problemset } from './types';
...
export class ApiClient {
        ...
        this.axiosInstance.interceptors.response.use(
            ...
            async (error: AxiosError<ApiError>) => {
                ...
                if (error.response?.status === 401 && originalRequest) {
                    ...
                    return Promise.reject(new Error('Invalid or expired token. Please set a new one.'));
                }
    ...
    async getProblems(cursor?: string, keyword?: string, problemsetId?: number): Promise<{ problems: ProblemBrief[], next: string | null }> {
        const params: Record<string, any> = {};
        if (cursor) params.cursor = cursor;
        if (keyword) params.keyword = keyword;
        if (problemsetId) params.problemset_id = problemsetId;

        const response = await this.axiosInstance.get<{ problems: ProblemBrief[], next: string | null }>('/problem/', { params });
        return response.data;
    }

    async getProblemDetails(problemId: number): Promise<Problem> {
        const response = await this.axiosInstance.get<Problem>(`/problem/${problemId}`);
        return response.data;
    }
    ...
    async getUserProblemsets(): Promise<{ problemsets: Problemset[] }> {
        ...
        const response = await this.axiosInstance.get<{ problemsets: Problemset[] }>('/user/problemsets');
        return response.data;
    }

    async getProblemsetDetails(problemsetId: number): Promise<Problemset> {
        const response = await this.axiosInstance.get<Problemset>(`/problemset/${problemsetId}`);
        return response.data;
    }

    // --- Submission Endpoints ---

    async submitCode(problemId: number, language: string, code: string, isPublic: boolean = false): Promise<{ id: number }> {
    ...
    async getSubmissions(cursor?: string, username?: string, problemId?: number, status?: string, lang?: string): Promise<{ submissions: SubmissionBrief[], next: string | null }> {
        const params: Record<string, any> = {};
        if (cursor) params.cursor = cursor;
        ...
        const response = await this.axiosInstance.get<{ submissions: SubmissionBrief[], next: string | null }>('/submission/', { params });
        return response.data;
    }

    async getSubmissionDetails(submissionId: number): Promise<Submission> {
        const response = await this.axiosInstance.get<Submission>(`/submission/${submissionId}`);
        return response.data;
    }

    async getSubmissionCode(codeUrl: string): Promise<string> {
        try {
            const response = await axios.get<string>(codeUrl, { baseURL: this.axiosInstance.defaults.baseURL?.replace('/api/v1', '') });
            return response.data;
        } catch (error) {
    ...
    // --- User Endpoints ---
    async getUserProfile(): Promise<Profile> { // Return specific Profile type
